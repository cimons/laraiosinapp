...
namespace Cimons\LaraIosInApp\iTunes;
...
class PurchaseItem implements ArrayAccess
{
    ...
    public function parseData()
    {
        ...
        if (array_key_exists('quantity', $this->raw_data)) {
            $this->quantity = $this->raw_data['quantity'];
        }
        ...
        if (array_key_exists('purchase_date_ms', $this->raw_data)) {
            $this->purchase_date = Carbon::createFromTimestampUTC(intval(round($this->raw_data['purchase_date_ms'] / 1000)));
        }

        if (array_key_exists('original_purchase_date_ms', $this->raw_data)) {
            $this->original_purchase_date = Carbon::createFromTimestampUTC(intval(round($this->raw_data['original_purchase_date_ms'] / 1000)));
        }

        if (array_key_exists('expires_date_ms', $this->raw_data)) {
            $this->expires_date = Carbon::createFromTimestampUTC(intval(round($this->raw_data['expires_date_ms'] / 1000)));
        } elseif (array_key_exists('expires_date', $this->raw_data) && is_numeric($this->raw_data['expires_date'])) {
            $this->expires_date = Carbon::createFromTimestampUTC(intval(round((int) $this->raw_data['expires_date'] / 1000)));
        }

        if (array_key_exists('cancellation_date_ms', $this->raw_data)) {
            $this->cancellation_date = Carbon::createFromTimestampUTC(intval(round($this->raw_data['cancellation_date_ms'] / 1000)));
        }
